package com.meskiep.vaithat.core.custom.drawview
...
import com.meskiep.vaithat.data.model.draw.DrawDraw
import com.meskiep.vaithat.data.model.draw.DrawableDraw
import com.meskiep.vaithat.data.model.draw.TextDraw
import com.meskiep.vaithat.listener.listenerdraw.BitmapDrawIcon
import com.meskiep.vaithat.listener.listenerdraw.DeleteEvent
import com.meskiep.vaithat.listener.listenerdraw.EditEvent
import com.meskiep.vaithat.listener.listenerdraw.FlipEvent
import com.meskiep.vaithat.listener.listenerdraw.OnDrawListener
import com.meskiep.vaithat.listener.listenerdraw.ZoomEvent
...
//                }
    ...
    private fun addDrawImmediately(draw: DrawableDraw, position: Int) {
        ...
        val drawableWidth = draw.drawable.intrinsicWidth.coerceAtLeast(1)
        val drawableHeight = draw.drawable.intrinsicHeight.coerceAtLeast(1)

        val widthScaleFactor = width.toFloat() / drawableWidth * 0.7f
        val heightScaleFactor = height.toFloat() / drawableHeight * 0.7f
            ...
            if (isShowIcons) {
                ...
                for (i in 0 until iconList.size) {
                    ...
                    when (icon.positionDefault) {
                        ...
                        DrawKey.TOP_RIGHT -> setupMatrix(icon, x2, y2, rotation)
                        DrawKey.BOTTOM_LEFT -> setupMatrix(icon, x3, y3, rotation)
    ...
    private fun setupDefaultIcons() {
        val deleteIcon = BitmapDrawIcon(
            ContextCompat.getDrawable(context, R.drawable.ic_delete_draw), DrawKey.TOP_RIGHT
        )
        deleteIcon.event = DeleteEvent()

        val zoomIcon = BitmapDrawIcon(
            ContextCompat.getDrawable(context, R.drawable.ic_rotation_draw), DrawKey.BOTTOM_RIGHT
        )
        zoomIcon.event = ZoomEvent()

        val flipHorizontalIcon = BitmapDrawIcon(
            ContextCompat.getDrawable(context, R.drawable.ic_flip_draw_horizontal), DrawKey.TOP_LEFT
        )
        flipHorizontalIcon.event = FlipEvent(DrawKey.FLIP_HORIZONTALLY)

        val flipVerticalIcon = BitmapDrawIcon(
            ContextCompat.getDrawable(context, R.drawable.ic_flip_draw_vertical), DrawKey.BOTTOM_LEFT
        )
        flipVerticalIcon.event = FlipEvent(DrawKey.FLIP_VERTICALLY)

        iconList.clear()
        iconList.add(deleteIcon)
        iconList.add(zoomIcon)
        iconList.add(flipHorizontalIcon)
        iconList.add(flipVerticalIcon)
    }
