package com.meskiep.vaithat.core.share.whatsapp
...
abstract class WhatsappSharingActivity<DB : ViewBinding> : BaseActivity<DB>() {
    ...
    fun addToWhatsapp(sp: StickerPack, onSuccess: () -> Unit) {
        if (sp.stickers.size >= 3) {
            addStickerPackageToWhatsApp(sp, onSuccess)
        } else {
            showErrorDialog()
        }
    }
    ...
    private fun addStickerPackageToWhatsApp(sp: StickerPack, onSuccess: () -> Unit) {
        val intent = Intent()
